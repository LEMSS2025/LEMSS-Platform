...
class Game:
    ...
    def __init__(self, query_info: dict, agents: list, ranker: ranker, llm: LLM, max_tokens: int, rounds: int,
        ...
        self.__players = [agent.get_player(query_info[QUERY_DF_QUERY_COLUMN]) for agent in agents]
        ...
        for player in self.__players:
            self.__game_history.loc[len(self.__game_history)] = [0, player.get_name(), self.__init_doc, None, None, None, None, None]
    ...
    def generate_documents(self) -> list:
        ...
        try:
            ...
            documents_prompts = [player.generate_document(self.__llm, self.__max_tokens, self.__init_doc,
                                                          force_max_tokens=self.__force_max_tokens)
                                 for player in self.__players]
    ...
    def rank_documents(self, documents_prompts: list, docnos: list = None) -> list:
        ...
        try:
            ...
            return sorted(zip(self.__players, documents, ranks, scores, non_cleaned_documents, user_prompts, system_prompts),
    ...
    def create_round_history(self, ranked_players: list) -> pd.DataFrame:
        ...
        try:
            ...
            for player, doc, rank, score, not_clean_doc, user_prompt, system_prompt in ranked_players:
                round_df.loc[len(round_df)] = [self.__round, player.get_name(), doc, not_clean_doc, rank, score, user_prompt, system_prompt]
    ...
    def get_game_history(self) -> pd.DataFrame:
        ...
        """
        self.__game_history[HISTORY_QUERY_ID_COLUMN] = self.__query_id
        self.__game_history[HISTORY_GAME_ID_COLUMN] = self.__query_id
